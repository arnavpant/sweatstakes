...
export function PhotoGallery({ photos }: { photos: Photo[] }) {
  if (photos.length === 0) {
    return (
      <div className="bg-[#0b1a3a] border border-white/10 rounded-md p-6 flex flex-col items-center text-center space-y-2">
      ...
      <div className="bg-[#0b1a3a] border border-white/10 rounded-md p-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          {photos.slice(0, 6).map((p) => (
            <div
              key={p.id}
              className="aspect-square overflow-hidden rounded-md bg-[#13254f]"
            >
              <img
                src={p.selfieUrl ?? p.photoUrl}
                alt="Check-in"
